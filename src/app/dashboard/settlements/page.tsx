...
export default function SettlementsPage() {
  ...
  const handleSettlementAction = (settlementId: string, action: 'complete' | 'cancel' | 'view') => {
    if (action === 'view') {
      setSelectedSettlementId(settlementId);
    } else {
      // إعادة تحميل البيانات بعد الإجراء
      fetchSettlements();
    }
  };
  ...
  const handleDetailsAction = (action: 'complete' | 'cancel') => {
    // إعادة تحميل البيانات بعد الإجراء
    fetchSettlements();
  };
                  ...
                  {settlements.map((settlement) => (
                    <tr key={settlement.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <button 
                          onClick={() => handleSettlementAction(settlement.id, 'view')}
