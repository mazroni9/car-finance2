/**
 * سكريبت تطبيق الإسكيم الموحد (combined_schema.sql)
...
async function applyCombinedSchema() {
  try {
    console.log('🚀 بدء تطبيق الإسكيم الموحد (combined_schema.sql)...');
    
    // قراءة ملف الإسكيم الموحد
    const schemaPath = path.join(__dirname, '../sql/combined_schema.sql');
...
async function verifyData() {
  try {
    ...
    // التحقق من جدول installment_rules
    ...
    if (rulesError) {
      console.error('❌ خطأ في التحقق من جدول installment_rules:', rulesError);
    } else {
      console.log('✅ جدول installment_rules يعمل بشكل صحيح');
    }
...
module.exports = { applyCombinedSchema }; 