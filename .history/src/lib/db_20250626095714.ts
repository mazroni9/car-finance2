...
export const validateCarData = (data: CarData) => {
  const requiredFields = ['make', 'model', 'year', 'price'] as const;
  for (const field of requiredFields) {
    if (!data[field]) {
      throw new Error(`Missing required field: ${field}`);
    }
    // تحقق إضافي من نوع البيانات
    if (field === 'year' && (typeof data.year !== 'number' || data.year < 1900 || data.year > new Date().getFullYear() + 1)) {
      throw new Error('Invalid year value');
    }
    if (field === 'price' && (typeof data.price !== 'number' || data.price <= 0)) {
