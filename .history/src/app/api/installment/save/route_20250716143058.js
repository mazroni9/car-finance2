...
export async function POST(request) {
  ...
  const tracking_cost = quantity * 25; // أو 0 إذا لم يكن مطلوباً
  const guarantee_contract_cost = quantity * 50; // أو 0 إذا لم يكن مطلوباً
  const inspection_cost = quantity * 300; // أو 0 إذا لم يكن مطلوباً
  const profit_per_car = total_sale_price - price_category;

  // تجهيز البيانات للإدخال (فقط الأعمدة المطلوبة)
  const insertData = {
    price_category,
    duration_months,
