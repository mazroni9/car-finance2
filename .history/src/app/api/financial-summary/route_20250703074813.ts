  ...
  try {
    ...
    const summary = rules.reduce((acc, rule) => {
      try {
        ...
        // استخدام القيم المباشرة من الجدول
        const downPayment = Number(rule.down_payment_value) || 0;     // الدفعة الأولى
        const lastPayment = Number(rule.last_payment_value) || 0;     // الدفعة الأخيرة
        const durationMonths = Number(rule.duration_months) || 12;

        // حساب عدد الدفعات الأخيرة في السنة الواحدة
        const lastPaymentsPerYear = Math.floor(12 / durationMonths) * lastPayment;
        ...
        return {
          ...
          total_down_payments: acc.total_down_payments + (downPayment * quantity),
