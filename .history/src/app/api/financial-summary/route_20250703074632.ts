  ...
  try {
    ...
    const summary = rules.reduce((acc, rule) => {
      try {
        ...
        const firstPayment = Number(rule.first_payment) || 0; // الدفعة الأولى
        const lastPayment = Number(rule.last_payment) || 0;   // الدفعة الأخيرة
        ...
        return {
          ...
          total_down_payments: acc.total_down_payments + (firstPayment * quantity),
