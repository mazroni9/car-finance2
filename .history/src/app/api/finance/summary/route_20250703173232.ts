...
export async function GET() {
  const { data, error } = await supabase
    ...
    .select(`
      ...
      total_last_payments_per_year:sum(last_payment_value * quantity),
      total_annual_profit_before_costs:sum(annual_profit_before_costs * quantity),
      total_profit_full_period:sum((monthly_installment * term_months + down_payment_value + last_payment_value - car_price) * quantity),
