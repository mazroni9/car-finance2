...
export async function GET() {
  try {
    ...
    // جلب عدد السيارات المؤجرة (السيارات التي تم تأجيرها)
    const { count: leasedCarsCount, error: leasedCarsError } = await supabase
      .from('car_showcase')
      .select('*', { count: 'exact', head: true })
      .eq('status', 'leased');

    if (leasedCarsError) {
      console.error('Error fetching leased cars count:', leasedCarsError);
    }
...
return NextResponse.json({
  ...
  total_service_fees,
  leased_cars_count: leasedCarsCount || 0
});
