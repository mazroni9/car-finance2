...
export async function GET() {
  const { data, error } = await supabase
    ...
    .select(`
      total_monthly_installments:sum(monthly_installment),
      total_annual_income:sum(monthly_installment * 12),
      total_down_payments:sum(down_payment_value),
      total_last_payments_per_year:sum(last_payment_value),
      total_annual_profit_before_costs:sum(annual_profit_before_costs),
      total_profit_full_period:sum(monthly_installment * term_months + down_payment_value + last_payment_value - car_price),
  ...
  const safe = (num: number | null | undefined) => num ?? 0;
