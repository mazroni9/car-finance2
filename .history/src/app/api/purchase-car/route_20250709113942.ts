...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { showroomWalletId, platformWalletId, totalPrice } = body;

    const userSession = await supabaseClient().auth.getUser();
    if (!userSession?.data?.user?.id) {
      throw new Error("المستخدم غير مصرح");
    }
    const buyerUserId = userSession.data.user.id;

    // ✅ جلب محفظة المشتري
    const buyerWallet = await getWalletByUserId(buyerUserId);
