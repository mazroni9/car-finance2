  ...
  const handleSave = () => {
    if (editData) {
      ...
      if (editData.price_category <= 25000) {
        ...
        const baseAmount = editData.price_category - editData.first_payment - editData.last_payment;
  ...
  const applyFormulasToData = (data: FinanceData[]) => {
    return data.map(item => {
      ...
      if (item.price_category <= 25000) {
        ...
        // للفئات 30,000 وما فوق: الربح من (الفئة السعرية - الدفعة الأولى - الدفعة الأخيرة)
        const baseAmount = item.price_category - item.first_payment - item.last_payment;
      ...
      <div className="mt-8 p-4 bg-gray-50 border border-gray-200 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-gray-600">
          <div>
            ...
            <p className="mb-1"><strong>القسط الشهري =</strong> (سعر البيع بالتقسيط - الدفعة الأولى - الدفعة الأخيرة + تكلفة التأمين + التكاليف الثابتة السنوية) ÷ مدة التمويل</p>
