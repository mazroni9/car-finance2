...
export default function ProfessionalFinanceTable({ data, onDataChange }: ProfessionalFinanceTableProps) {
  ...
  const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    // ربط نسبة الربح تلقائيًا حسب مدة الإيجار
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const newData: FinanceData = {
      ...
      last_payment: form.price_category * 0.1, // افتراضي 10%
      profit_after: 0,
      annual_profit: 0,
    ...
    setForm({ ...form, car_count: 1 }); // إعادة تعيين عدد السيارات فقط
  };

  return (
    <div className="space-y-6">
      {/* الجداول المجمعة */}
      {/* أضف النموذج قبل الجداول المجمعة */}
      <form onSubmit={handleFormSubmit} className="flex flex-wrap gap-2 items-end bg-white rounded-lg shadow p-3 mb-6">
        ...
        <input name="last_payment_percent" type="number" step="0.01" min={0} max={1} value={form.last_payment_percent} onChange={handleFormChange} className="px-2 py-1 border rounded text-sm w-20" placeholder="نسبة تمديد الإيجار" />
        <input name="extra_annual_fee" type="number" min={0} value={form.extra_annual_fee} onChange={handleFormChange} className="px-2 py-1 border rounded text-sm w-24" placeholder="رسوم سنوية إضافية" />
