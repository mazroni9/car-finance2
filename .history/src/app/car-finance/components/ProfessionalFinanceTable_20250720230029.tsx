  ...
  const handleSave = () => {
    if (editData) {
      ...
      if (editData.duration_months === 12) {
        ...
        annualProfit = profit + serviceFee;
      } else {
        // للفترات الأطول: نحسب الربح السنوي فقط + رسوم الخدمة كاملة (تُحصل فوراً)
        const annualProfitFromPeriod = profit * (12 / editData.duration_months);
        annualProfit = annualProfitFromPeriod + serviceFee;
      }
  ...
  const applyFormulasToData = (data: FinanceData[]) => {
    return data.map(item => {
      ...
      if (item.duration_months === 12) {
        ...
        // للفترات الأطول: نحسب الربح السنوي فقط + رسوم الخدمة كاملة (تُحصل فوراً)
        const annualProfitFromPeriod = profit * (12 / item.duration_months);
        annualProfit = annualProfitFromPeriod + serviceFee;
      }
