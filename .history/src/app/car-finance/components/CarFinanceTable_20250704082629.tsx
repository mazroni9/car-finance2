...
interface InstallmentRule {
  ...
  annual_profit_before_costs: number;
        ...
        <table className="w-full divide-y divide-gray-200 text-gray-900">
          <thead className="bg-blue-700">
            <tr>
              ...
              <th className="px-4 py-4 text-center text-sm font-bold text-white uppercase tracking-wider">الربح السنوي قبل التكاليف</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {rules.map((rule, index) => (
              <tr key={rule.id} className={`${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-blue-50 transition-colors`}>
                ...
                <td className="px-4 py-4 text-center whitespace-nowrap text-sm font-medium">{formatNumber(rule.annual_profit_before_costs)} SAR</td>
