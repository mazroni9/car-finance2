...
export default function CarFinancePage() {
  ...
  const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    let newForm = { ...form, [name]: name === 'car_count' || name === 'price_category' || name === 'duration_months' || name === 'last_payment' ? Number(value) : value };
    if (name === 'duration_months') {
      newForm.profit_percent = profitPercentByDuration[Number(value)] || 30;
    }
    if (name === 'price_category') {
      newForm.last_payment = Math.round(Number(value) * 0.1);
    }
    setForm(newForm);
  };
        ...
        <div className="space-y-8">
          ...
          <div className="glass-card p-4 mb-6 flex flex-col md:flex-row md:items-end md:justify-between gap-4 bg-gradient-to-br from-blue-50 to-blue-100 shadow-md">
            <form onSubmit={handleFormSubmit} className="flex flex-wrap gap-2 items-end">
              ...
              <input name="last_payment" type="number" min={0} value={form.last_payment} onChange={handleFormChange} className="px-2 py-1 border rounded text-sm w-20" placeholder="الدفعة الأخيرة" />
