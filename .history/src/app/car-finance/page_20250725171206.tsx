        ...
        <div className="space-y-8">
          ...
          <div className="glass-card p-4 mb-6 flex flex-col md:flex-row md:items-end md:justify-between gap-4 bg-gradient-to-br from-blue-50 to-blue-100 shadow-md">
            <form onSubmit={handleFormSubmit} className="flex flex-wrap gap-2 items-end">
              ...
              <select name="last_payment_percent" value={form.last_payment_percent} onChange={handleFormChange} className="px-2 py-1 border rounded text-sm w-20">
                {lastPaymentPercents.map((p) => (
                  <option key={p} value={p}>{Math.round(p * 100)}%</option>
                ))}
              </select>
              <input name="last_payment" type="text" value={form.last_payment.toLocaleString()} readOnly className="px-2 py-1 border rounded bg-gray-100 text-center font-bold text-sm w-20" tabIndex={-1} />
