...
export default function CarFinancePage() {
  ...
  const calculateSummary = (data: FinanceData[]) => {
    ...
    // حساب إجمالي الربح السنوي بطريقة موحدة (يشمل رسوم الخدمة)
    const total_annual_profit_before_costs = data.reduce((sum, item) => {
      // للفترات 12 شهر: نأخذ الربح كاملاً + رسوم الخدمة كاملة
      if (item.duration_months === 12) {
        return sum + (item.profit_after || 0) + (item.first_payment || 0);
      }
      // للفترات الأطول: نحسب الربح السنوي فقط + رسوم الخدمة كاملة (تُحصل فوراً)
      else {
        const annualProfit = (item.profit_after || 0) * (12 / item.duration_months);
        const serviceFee = item.first_payment || 0; // رسوم الخدمة تُحصل فوراً
        return sum + annualProfit + serviceFee;
      }
    }, 0);
