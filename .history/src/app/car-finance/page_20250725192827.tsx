...
export default function CarFinancePage() {
  ...
  // قائمة الفئات السعرية المعروفة
  const knownPriceCategories = [20000, 25000, 30000, 35000, 40000, 45000];
    ...
    <div className="container mx-auto py-8">
      ...
      <form onSubmit={handleFormSubmit} className="bg-white rounded-lg shadow-md p-6 mb-8 flex flex-col md:flex-row items-end gap-4">
        <div className="flex flex-col">
          ...
          <select
            name="price_category"
            value={knownPriceCategories.includes(form.price_category) ? form.price_category : 'other'}
            onChange={e => {
              if (e.target.value === 'other') {
                setForm({ ...form, price_category: customPrice });
              } else {
                setForm({ ...form, price_category: Number(e.target.value) });
              }
            }}
          ...
          {/* حقل إدخال مخصص يظهر فقط عند اختيار أخرى */}
          {(!knownPriceCategories.includes(form.price_category)) && (
            <input
              type="number"
              min={0}
              ...
              placeholder="أدخل الفئة السعرية المخصصة"
              title="فئة سعرية مخصصة"
            />
          )}
        </div>
        <div className="flex flex-col">
