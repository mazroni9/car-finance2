...
export default function DealerDashboard() {
  ...
  async function fetchWallet() {
    try {
      ...
      if (error) {
        ...
        if (data) {
          console.log('معرف التاجر في المحفظة:', data.user_id);
  ...
  const availableFunds =
    wallet.balance + (wallet.balance_limit - wallet.balance_approved);

  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-8 text-center">
        <h2 className="text-2xl font-bold mb-2">أهلا بك في صفحتك الخاصة يا {wallet.name}!</h2>
        <p className="text-gray-600">يمكنك تنفيذ صفقات ومعرفة رصيدك من هنا</p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-100">
              <tr>
                <th className="px-4 py-2 text-right text-gray-900 font-bold">التاريخ</th>
                <th className="px-4 py-2 text-right text-gray-900 font-bold">المبلغ</th>
                <th className="px-4 py-2 text-right text-gray-900 font-bold">النوع</th>
                <th className="px-4 py-2 text-right text-gray-900 font-bold">الوصف</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {transactions.map((tx) => (
                <tr key={tx.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2 text-gray-900">{new Date(tx.created_at).toLocaleDateString('ar-SA')}</td>
                  <td className="px-4 py-2 text-gray-900">{tx.amount.toLocaleString()} ريال</td>
                  <td className="px-4 py-2 text-gray-900">{tx.type === 'credit' ? 'إيداع' : 'سحب'}</td>
