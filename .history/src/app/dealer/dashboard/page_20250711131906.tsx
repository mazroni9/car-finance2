      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-green-100 rounded-lg p-4 text-center">
          ...
          <div className="text-2xl font-extrabold text-green-900">
            {transactions.filter(tx => tx.description?.includes('عمولة') || tx.type === 'platform_commission').reduce((sum, tx) => sum + Math.abs(tx.amount), 0).toLocaleString()} ريال
          </div>
        </div>
        <div className="bg-yellow-100 rounded-lg p-4 text-center">
          ...
          <div className="text-2xl font-extrabold text-yellow-900">
            {transactions.filter(tx => tx.description?.includes('نقل ملكية') || tx.type === 'ownership_transfer_fee').reduce((sum, tx) => sum + Math.abs(tx.amount), 0).toLocaleString()} ريال
          </div>
        </div>
        <div className="bg-gray-100 rounded-lg p-4 text-center">
          <div className="text-lg font-bold text-gray-800 mb-1">إجمالي الرسوم الأخرى</div>
          <div className="text-2xl font-extrabold text-gray-900">
            {transactions.filter(tx => tx.type?.includes('fee') && tx.type !== 'ownership_transfer_fee').reduce((sum, tx) => sum + Math.abs(tx.amount), 0).toLocaleString()} ريال
          </div>
        </div>
      </div>
          ...
          <table className="min-w-full border-collapse">
            ...
            <tbody className="bg-white divide-y">
              {transactions.length === 0 && (
                <tr>
                  <td colSpan={5} className="text-center py-6 text-gray-500">لا توجد عمليات مالية مسجلة بعد.</td>
                </tr>
              )}
              {transactions.length > 0 && (() => {
                let runningBalance = wallet?.balance ?? 0;
                // العمليات مرتبة من الأحدث للأقدم، نحتاج عكسها لحساب الرصيد التراكمي بشكل صحيح
                const txs = [...transactions].sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
                // سنحسب الرصيد بعد كل عملية
                return txs.map((tx, idx) => {
                  // العملية credit تزيد الرصيد، debit تنقصه
