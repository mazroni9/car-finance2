...
export default function DealerDashboard() {
  ...
  async function getUser() {
    ...
    if (error || !data?.user) {
      console.error('❌ لم يتم العثور على مستخدم مسجل دخول');
      setUserId(null);
      setIsLoading(false);
    } else {
    ...
    return (
      <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-lg">
        <div className="text-center">
          <div className="mb-4">
            <img src="/images/wallet-icon.png" alt="Wallet" className="w-24 h-24 mx-auto" />
          </div>
          <h2 className="text-xl font-bold mb-4">المحفظة غير مفعلة</h2>
          <p className="text-gray-600 mb-2">
            حالة المحفظة: {!wallet ? 'غير موجودة' : wallet.status === 'pending' ? 'قيد المراجعة' : 'غير مفعلة'}
          </p>
          <p className="text-gray-600 mb-6">
            {!wallet 
              ? 'لم يتم العثور على محفظة. يرجى التواصل مع الإدارة لإنشاء محفظة جديدة.' 
              : 'المحفظة موجودة ولكنها تحتاج إلى تفعيل. يرجى التواصل مع الإدارة لتفعيل محفظتك.'}
          </p>
          <div className="space-y-4">
            <button
              onClick={() => {
                const adminContact = `/contact-admin?user=${encodeURIComponent(userId || '')}&walletStatus=${wallet?.status || 'not_found'}`;
