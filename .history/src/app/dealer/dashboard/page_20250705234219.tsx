    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold text-gray-900">📜 سجل المشتريات</h3>
          <div className="flex items-center gap-2">
            <span className="text-lg">📋</span>
            <span className="text-blue-600 font-bold">سجل المشتريات</span>
          </div>
        </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {transactions.map((tx) => (
                <tr key={tx.id} className="hover:bg-gray-50">
                  <td className="px-6 py-3 text-gray-900">{new Date(tx.created_at).toLocaleDateString('ar-SA')}</td>
                  <td className="px-6 py-3 text-gray-900">{tx.amount.toLocaleString()} ريال</td>
                  <td className="px-6 py-3 text-gray-900">{tx.type === 'credit' ? 'إيداع' : 'سحب'}</td>
                  <td className="px-6 py-3 text-gray-900">{tx.description}</td>
                </tr>
              ))}
            </tbody>
          </table>
