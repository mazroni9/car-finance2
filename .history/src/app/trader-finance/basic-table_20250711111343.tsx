...
export default function BasicTable({ entries, details }: BasicTableProps) {
  ...
  return (
    <div className="mb-8">
      <h2 className="text-xl font-semibold mb-4 text-center">جدول الاشتراكات الأساسية</h2>
            ...
            {entries.map((entry) => {
              ...
              return (
                <tr key={entry.financingRate}>
                  <td className="border p-1 text-center">{entry.financingRate}%</td>
                  <td className="border p-1 text-center">{entry.monthlySubscription.toLocaleString()}</td>
                  <td className="border p-1 text-center">{subscribers}</td>
