      ...
      <table className="min-w-full w-full text-sm text-center border border-gray-200 rounded-lg overflow-hidden">
        <thead>
          <tr className="bg-yellow-100 text-black font-bold">
            ...
            <th className="border border-gray-200 px-2 py-2">مجموع الإيجار المتأخر</th>
            ...
            <th className="border border-gray-200 px-2 py-2">مجموع الربح الممتد</th>
        ...
        <tbody>
          {summaryByCategory.map((row) => (
            <tr key={row.priceCategory} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="border border-gray-200 px-2 py-2 text-black">{(600 * (row.extendedLease / (row.monthlyRent || 1))).toLocaleString(undefined, {maximumFractionDigits: 2})}</td>
          ...
          <tr className="bg-green-100 font-bold">
            ...
            <td className="border border-gray-200 px-2 py-2 text-blue-800 font-bold">
              {summaryByCategory.reduce((sum, row) => sum + row.quantity, 0) > 0 ? Math.round(((summaryByCategory.reduce((sum, row) => sum + row.baseProfit, 0) + summaryByCategory.reduce((sum, row) => sum + row.adminFees, 0)) / summaryByCategory.reduce((sum, row) => sum + (row.priceCategory * row.quantity), 0)) * 100) : 0}%
            </td>
            <td className="border border-gray-200 px-2 py-2 text-black font-bold">
            ...
            <td className="border border-gray-200 px-2 py-2 text-black font-bold">
              {summaryByCategory.reduce((sum, row) => sum + (600 * (row.extendedLease / (row.monthlyRent || 1))), 0).toLocaleString(undefined, {maximumFractionDigits: 2})}
            </td>
          ...
          <tr className="bg-gray-50">
            <td className="border px-2 text-black text-sm" colSpan={9}>
              <div className="text-center py-2">
                ...
                <span className="text-green-600 font-mono">((مجموع الربح الأساسي + مجموع الرسوم الإدارية) ÷ تكلفة الشراء) × 100</span>
                <br />
                الناتج نسبة مئوية
              </div>
            </td>
