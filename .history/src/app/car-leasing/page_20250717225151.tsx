...
function calculateRow(form: any, month: string) {
  ...
  // الإيجار الممتد = (قيمة الدفعة الأخيرة × نسبة الربح) + الرسوم السنوية + التأمين
  const extended_lease = (last_payment_value * profit_target_percent) + extra_annual_fee + insurance_value;
...
export default function CarLeasingPage() {
  ...
  const handleSave = () => {
    setRows((prev) => [...prev, calculateRow(form, selectedMonth)]);
  };
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              <label htmlFor="profitTargetPercent" className="block text-sm font-medium text-gray-900">نسبة الربح المستهدفة</label>
              <input
            ...
            <table className="min-w-full w-full text-sm text-center border border-blue-200 rounded-lg overflow-hidden">
              <thead>
                <tr className="bg-blue-800 text-white">
                  ...
                  <th className="border border-blue-200 px-2 font-bold">الإيجار الممتد</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="border border-blue-200 px-2 text-purple-900 bg-purple-100 font-bold">{((row.last_payment_value * 0.3) + row.extra_annual_fee + (2000 * row.quantity)).toLocaleString(undefined, {maximumFractionDigits: 2})}</td>
                  </tr>
                ))}
                {rows.length > 0 && (
                  <tr className="bg-green-200 font-bold">
                    <td className="border border-blue-200 px-2 text-black" colSpan={rows[0] ? 1 : 0}></td>
                    <td className="border border-blue-200 px-2 text-black"></td>
                    <td className="border border-blue-200 px-2 text-black"></td>
                    <td className="border border-blue-200 px-2 text-black"></td>
                    <td className="border border-blue-200 px-2 text-black"></td>
                    <td className="border border-blue-200 px-2 text-black"></td>
                    ...
                    <td className="border border-blue-200 px-2 text-green-800 bg-green-100 font-extrabold text-lg">
                      {rows.reduce((sum, r) => sum + r.monthly_rent, 0).toLocaleString(undefined, {maximumFractionDigits: 2})}
                    </td>
                    <td className="border border-blue-200 px-2 text-green-900 bg-green-200 font-extrabold">
                      {rows.reduce((sum, r) => sum + r.monthly_rent * 12, 0).toLocaleString(undefined, {maximumFractionDigits: 2})}
                    </td>
                    <td className="border border-blue-200 px-2 text-purple-900 bg-purple-200 font-extrabold">
                      {rows.reduce((sum, r) => sum + ((r.last_payment_value * 0.3) + r.extra_annual_fee + (2000 * r.quantity)), 0).toLocaleString(undefined, {maximumFractionDigits: 2})}
                    </td>
                  </tr>
                )}
                {rows.length === 0 && (
