      ...
      <table className="min-w-full w-full text-sm text-center border">
        <thead>
          <tr>
            ...
            <th className="border px-2 text-black font-bold">ROI المحتمل عند استكمال العقد</th>
          </tr>
        </thead>
        <tbody>
          {summaryByCategory.map((row) => (
            <tr key={row.priceCategory}>
              ...
              <td className="border px-2 text-black font-bold text-blue-800">
                {Math.round(((row.monthlyRent * 12 + row.adminFees) / (row.priceCategory * row.quantity)) * 100)}%
              </td>
              ...
              <td className="border px-2 text-black font-bold text-green-800">
                {Math.round(((row.monthlyRent * 12 + row.extendedLease + row.adminFees) / (row.priceCategory * row.quantity)) * 100)}%
              </td>
            </tr>
          ))}
          {/* صف المجاميع */}
          <tr className="bg-green-200 font-bold">
            ...
            <td className="border px-2 text-black font-bold text-blue-800">
              {Math.round(((summaryByCategory.reduce((sum, row) => sum + row.monthlyRent * 12, 0) + 
                 summaryByCategory.reduce((sum, row) => sum + row.adminFees, 0)) / 
                 summaryByCategory.reduce((sum, row) => sum + (row.priceCategory * row.quantity), 0)) * 100)}%
            </td>
            <td className="border px-2 text-black font-bold">
          ...
          <tr className="bg-gray-100">
            <td className="border px-2 text-black text-sm" colSpan={9}>
              <div className="text-center py-2">
                ...
                <span className="text-green-600 font-mono">((الإيجار السنوي المعتمد + الإيجار الممتد + الرسوم الإدارية) ÷ تكلفة الشراء) × 100</span>
              </div>
            </td>
          </tr>
        </tbody>
