...
function calculateRow(form: any) {
  ...
  const profit_value = price_category * profit_target_percent;
  const total_annual_fees = insurance_value + extra_annual_fee;
  const total_sale_price = price_category + insurance_value + extra_annual_fee + profit_value;
  const financed_amount = total_sale_price - down_payment_value - last_payment_value;
  const monthly_rent = (total_sale_price - down_payment_value - last_payment_value) / duration_months;
            ...
            <table className="min-w-full w-full text-sm text-center border">
              <thead>
                <tr>
                  ...
                  <th className="border px-2 text-black font-bold">الربح الكلي</th>
              ...
              <tbody>
                {rows.map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="border px-2 text-black">{row.profit_value.toLocaleString()}</td>
                ...
                {rows.length === 0 && (
                  <tr>
                    <td colSpan={16} className="text-gray-400 py-8">لا توجد بيانات بعد. أدخل البيانات أعلاه واضغط حفظ.</td>
                  </tr>
                )}
              </tbody>
            </table>
