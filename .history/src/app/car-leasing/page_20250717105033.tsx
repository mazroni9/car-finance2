            ...
            <table className="min-w-full text-sm text-center border">
              <thead>
                <tr>
                  ...
                  <th className="border px-2">الفئة السعرية</th>
                  <th className="border px-2">مدة التمويل</th>
                  <th className="border px-2">نسبة الربح</th>
                  <th className="border px-2">نسبة الدفعة الأولى</th>
                  <th className="border px-2">نسبة الدفعة الأخيرة</th>
                  <th className="border px-2">عدد السيارات</th>
                  <th className="border px-2">قيمة الدفعة الأولى</th>
                  <th className="border px-2">قيمة الدفعة الأخيرة</th>
                  <th className="border px-2">سعر البيع الإجمالي</th>
                  <th className="border px-2">المبلغ الممول</th>
                  <th className="border px-2">القسط الشهري</th>
                  <th className="border px-2">الربح لكل سيارة</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row, idx) => (
                  <tr key={row.id || idx}>
                    ...
                    <td className="border px-2">{row.price_category.toLocaleString()}</td>
                    <td className="border px-2">{row.duration_months}</td>
                    <td className="border px-2">{(row.profit_target_percent * 100).toFixed(0)}%</td>
                    <td className="border px-2">{(row.down_payment_percent * 100).toFixed(0)}%</td>
                    <td className="border px-2">{(row.last_payment_percent * 100).toFixed(0)}%</td>
                    <td className="border px-2">{row.quantity}</td>
                    <td className="border px-2">{row.down_payment_value.toLocaleString()}</td>
                    <td className="border px-2">{row.last_payment_value.toLocaleString()}</td>
                    <td className="border px-2">{row.total_sale_price.toLocaleString()}</td>
                    <td className="border px-2">{row.financed_amount.toLocaleString()}</td>
                    <td className="border px-2">{row.monthly_installment.toLocaleString(undefined, {maximumFractionDigits: 2})}</td>
                    <td className="border px-2">{row.profit_per_car.toLocaleString()}</td>
                  </tr>
                ))}
                {rows.length === 0 && (
                  <tr>
