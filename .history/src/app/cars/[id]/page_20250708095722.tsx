...
export default function CarDetailsPage({ params }: { params: { id: string } }) {
  ...
  // تكاليف الشراء
  const platformFee = 117;
  const showroomFee = 383;
  const total = car ? car.price + platformFee + showroomFee : 0;

  const handleConfirm = async () => {
    setConfirming(true);
    setError(null);
    // تنفيذ عملية الشراء (ربط مع التسويات)
    try {
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="md:w-1/2 space-y-6">
          ...
          <table className="w-full text-right border rounded-lg bg-white">
            <tbody>
              <tr><td className="font-bold">السعر</td><td>{typeof car.price === 'number' ? car.price.toLocaleString('ar-SA') + ' ريال' : 'غير متوفر'}</td></tr>
              <tr><td className="font-bold">اللون</td><td>{car.color}</td></tr>
              <tr><td className="font-bold">المسافة المقطوعة</td><td>{typeof car.mileage === 'number' ? car.mileage.toLocaleString('ar-SA') + ' كم' : 'غير متوفر'}</td></tr>
              <tr><td className="font-bold">الوقود</td><td>{car.fuel_type}</td></tr>
              <tr><td className="font-bold">القير</td><td>{car.transmission}</td></tr>
              <tr><td className="font-bold">الوصف</td><td>{car.description}</td></tr>
            </tbody>
          </table>
          ...
          <div className="mt-8">
            {!showCosts ? (
              ...
              <div className="bg-gray-50 p-4 rounded-lg border mt-4">
                <h3 className="font-bold mb-4 text-lg">ملخص التكاليف</h3>
                <ul className="mb-4 space-y-2">
                  <li>سعر السيارة: <span className="font-bold">{typeof car.price === 'number' ? car.price.toLocaleString('ar-SA') + ' ريال' : 'غير متوفر'}</span></li>
                  <li>عمولة المنصة: <span className="font-bold">{typeof platformFee === 'number' ? platformFee.toLocaleString('ar-SA') + ' ريال' : 'غير متوفر'}</span></li>
                  <li>عمولة المعرض: <span className="font-bold">{typeof showroomFee === 'number' ? showroomFee.toLocaleString('ar-SA') + ' ريال' : 'غير متوفر'}</span></li>
                  <li>الإجمالي: <span className="font-bold text-blue-600">{typeof total === 'number' ? total.toLocaleString('ar-SA') + ' ريال' : 'غير متوفر'}</span></li>
                </ul>
                {error && <div className="text-red-600 mb-2">{error}</div>}
                {success ? (
                  <div className="text-green-600 font-bold mb-2">تم تنفيذ العملية بنجاح!</div>
                ) : (
                  <button
