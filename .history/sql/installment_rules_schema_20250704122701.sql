...
CREATE TABLE installment_rules (
    ...
    annual_profit NUMERIC(12,2) GENERATED ALWAYS AS (
        CASE 
            WHEN term_months = 12 THEN profit_per_car * quantity
            WHEN term_months = 18 THEN (profit_per_car * quantity) / 1.5
            WHEN term_months = 24 THEN (profit_per_car * quantity) / 2
            ELSE 0
        END
    ) STORED,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
