-- إصلاح جدول installment_rules ليتطابق مع API route
...
CREATE TABLE installment_rules (
    ...
    total_monthly_installments NUMERIC(12,2) GENERATED ALWAYS AS (
        monthly_installment * duration_months
    ) STORED,
    
    financed_amount NUMERIC(12,2) GENERATED ALWAYS AS (
        possible_purchase_amount - down_payment_value - last_payment_value
    ) STORED,
    
    total_monthly_profit NUMERIC(12,2) GENERATED ALWAYS AS (
    ...
    annual_profit_before_costs NUMERIC(12,2) GENERATED ALWAYS AS (
        CASE 
            ...
            WHEN duration_months = 36 THEN (profit_per_car * quantity) / 3
            ELSE 0
        END
    ) STORED,
    
    total_purchase_cost NUMERIC(12,2) GENERATED ALWAYS AS (
        possible_purchase_amount + tracking_cost + guarantee_contract_cost + inspection_cost
    ) STORED,
...
(25000, 12, 0.30, 0.10, 0.20, 2500, 5000, 1, 2083.33, 25000, 25, 50, 300, 7500, 32500),
(30000, 24, 0.50, 0.15, 0.15, 4500, 4500, 1, 1312.50, 30000, 25, 50, 300, 15000, 45000),
(40000, 36, 0.60, 0.20, 0.10, 8000, 4000, 1, 1166.67, 40000, 25, 50, 300, 24000, 64000),
(50000, 12, 0.30, 0.10, 0.20, 5000, 10000, 2, 4166.67, 100000, 50, 100, 600, 15000, 65000); 