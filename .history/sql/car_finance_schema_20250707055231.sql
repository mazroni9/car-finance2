...
-- تحديث جدول السيارات
ALTER TABLE car_showcase 
ADD COLUMN seller_id UUID REFERENCES users(id),
ADD COLUMN showroom_id UUID REFERENCES showrooms(id);

-- تحديث البيانات التجريبية
UPDATE car_showcase
SET seller_id = (SELECT id FROM users WHERE username = 'mazbrothers'),
    showroom_id = (SELECT id FROM showrooms WHERE name = 'mazbrothers')
